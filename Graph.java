...
public class Graph {
	...
	//-----NEW---------
	// Method to create a subgraph for a node
	public Graph getSubgraph(int ego, Set<Integer> alters, Graph g) {
	    Graph subgraph = new Graph();
	    
	    // Add the node and its neighbors to the subgraph
	    subgraph.addVertex(ego);

	    for (int alter : alters) {
		      subgraph.addVertex(alter);
		      subgraph.addEdge(ego, alter);
		      subgraph.addEdge(alter, ego);
		      Set<Integer> AltersConnection = g.adj(alter);
		      
		      //check for any connections among alters
		      for(int curr : AltersConnection) {
		    	  if (AltersConnection.contains(curr)){
		    		  subgraph.addEdge(alter, curr);
		    		  subgraph.addEdge(curr, alter);
		    	  }
		      }
	    }
